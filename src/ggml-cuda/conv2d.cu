...
static __global__ void conv2d_pad_f16(const float* input, half* output, int iw, int ih, int pw, int ph, int px, int py, int total) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= total) {
        return;
    }
    const int x = i % pw - px, y = i / pw % ph - py, nc = i / (pw * ph);
    output[i] = __float2half((unsigned)x < (unsigned)iw && (unsigned)y < (unsigned)ih ? input[(nc * ih + y) * iw + x] : 0.0f);
}
...
static void conv2d_launch_implicit_gemm(const half* input, const half* weight, float* output, const conv_params& params, int split_k, dim3 grid, dim3 block, cudaStream_t stream) {
