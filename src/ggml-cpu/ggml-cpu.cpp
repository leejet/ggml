    ...
    switch (op->op) {
        ...
        case GGML_OP_MUL_MAT:
            if (src0->type == GGML_TYPE_I8) {
                ...
                const int64_t k                  = w4_kind != 0 ? 2 * src0->ne[0] : src0->ne[0];
                ...
                const bool packed_input          = src1->type == GGML_TYPE_I8 &&
                                                   packed_src != nullptr && src1->ne[0] == k &&
                                                   src1->ne[1] == packed_rows + packed_scale_rows;
                bool ok = (src1->type == GGML_TYPE_F32 || packed_input) && op->type == GGML_TYPE_F32 &&
                       ...
                       (convrot_group_size == 0 ||
                        (convrot_group_size <= 256 && k % convrot_group_size == 0));
                if (w4_kind == GGML_W4_CONVROT_KIND_W4A4) {
                    ok = ok && weight_scale != nullptr && weight_scale->type == GGML_TYPE_F32 &&
                         ggml_is_contiguous(weight_scale) &&
                         ggml_nelements(weight_scale) == src0->ne[1] &&
                         s_rel == nullptr;
                } else if (w4_kind == GGML_W4_CONVROT_KIND_W4A8) {
                    const ggml_tensor * s_channel = op->src[5];
                    ok = ok && k % 16 == 0 &&
                         weight_scale != nullptr && weight_scale->type == GGML_TYPE_F32 &&
                         ggml_is_contiguous(weight_scale) &&
                         ggml_nelements(weight_scale) == 16 &&
                         s_channel != nullptr && s_channel->type == GGML_TYPE_F32 &&
                         ggml_is_contiguous(s_channel) &&
                         ggml_nelements(s_channel) == src0->ne[1] &&
                         s_rel != nullptr && s_rel->type == GGML_TYPE_F8_E4M3 &&
                         ggml_is_contiguous(s_rel) &&
                         s_rel->ne[0] == k / 16 && s_rel->ne[1] == src0->ne[1];
                } else {
                    ok = ok && s_rel == nullptr &&
